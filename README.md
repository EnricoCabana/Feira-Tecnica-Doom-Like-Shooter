# 🎮 Battlehawk: Êxodos

Jogo de ação em 2D no estilo dos clássicos shooters old-school (tipo Doom clássico, só que em 2D), desenvolvido em **Unity 2D**.

Projeto desenvolvido para a **Feira Técnica 2024 do Colégio Univap**.

![Unity](https://img.shields.io/badge/Unity-2D-000000?logo=unity&logoColor=white)

## 📸 Preview

![Print 1](./screenshots/print1.png)
![Print 2](./screenshots/print2.png)
![Print 3](./screenshots/print3.png)

## 🎮 Sobre o jogo

Battlehawk: Êxodos é um jogo de ação/tiro em 2D inspirado nos clássicos shooters old-school como Doom, trazendo a atmosfera tensa e o ritmo acelerado do gênero para um visual 2D.

## 🛠️ Tecnologias

- Unity 2D
- C#

## 🚀 Como jogar

1. Baixe este repositório (botão **Code → Download ZIP**)
2. Extraia o arquivo baixado
3. Execute `BHE.exe`

> Este repositório contém apenas o executável do jogo. O código-fonte do projeto Unity não está disponível.
